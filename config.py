...
class Config(object):
    ...
    content_dim = 250 #描述的卷积核数 # rcnn = 500 cmm = 250
    num_classes = 19 # 类别
    ...
    vocab_size = 498681 # num of words 
    kernel_size = 1 #单尺度卷积核   # rcnn = 2 cnn=1
    kernel_sizes = [1,2,3,4,5,6] #多尺度卷积核
    title_seq_len = 50 # 标题长度,word为30 char为50
    content_seq_len = 250 #描述长度 word为120 char为250
    save_model_path = 'MultiCNNTextBNDeep.pt'
    type_='word' #word 和char
    all=False # 模型同时训练char和word

    embedding_path = './vector.300dim' # Embedding
    ...
    result_path='result/'+time.strftime(tfmt)+'.csv'
    ...
    model_paths = []#,'checkpoints/CNNText_tmp_0.402429167301']
...
def parse(self,kwargs,print_=True):
        ...
        ### word和char的长度不一样 ##
        if self.type_=='word':
            self.vocab_size = 498681 # num of words 
